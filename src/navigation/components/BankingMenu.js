...
const isBankingSeparatorRequired = urls => (
  urls.bankTransactionList || urls.bankReconciliation || urls.bankingRuleList || urls.bankFeeds
);

const isElectronicPaymentSeparatorRequired = urls => (urls.electronicPaymentCreate);
...
const getItems = (urls, onMenuLinkClick) => [
  ...
  isElectronicPaymentSeparatorRequired(urls) && <Navigation.Separator key="separator-electronic-payment" />,
  ...
  urls.transactionList && getMenuLink(urls.transactionList, 'Find transactions', onMenuLinkClick),
].filter(Boolean);
