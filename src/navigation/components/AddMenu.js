...
const isPayRunSeparatorRequired = (urls) => urls.payRunCreate;
...
const getItems = (urls, onMenuLinkClick) =>
  [
    ...
    urls.invoiceCreate &&
      getMenuLink(urls.invoiceCreate, 'Invoice', onMenuLinkClick),
    ...
    urls.payRunCreate &&
      getMenuLink(urls.payRunCreate, 'Pay run', onMenuLinkClick),
    ...
    urls.employeeCreate &&
      getMenuLink(urls.employeeCreate, 'Employee', onMenuLinkClick),
    urls.contactCreate &&
      getMenuLink(urls.contactCreate, 'Contact', onMenuLinkClick),
  ].filter(Boolean);

const AddMenu = ({ urls, onMenuSelect, onMenuLinkClick, className }) => (
  <ImageIconMenu
    image={addIconImage}
    tooltip="Create new"
