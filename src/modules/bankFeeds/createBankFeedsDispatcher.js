...
const createBankFeedsDispatcher = store => ({
  ...
  openBankFeedsLoginModal: () => store.dispatch({
    intent: SET_MODAL_TYPE,
  ...
  clearBankFeedsLogin: () => store.dispatch({
    intent: CLEAR_BANK_FEEDS_LOGIN,
  }),

  updateBankFeedsLoginDetails: ({ key, value }) => store.dispatch({
    intent: UPDATE_BANK_FEEDS_LOGIN,
    key,
    value,
  }),

  setIsTableLoading: isTableLoading => store.dispatch({
