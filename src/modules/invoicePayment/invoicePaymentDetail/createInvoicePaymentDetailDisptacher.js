...
const createInvoicePaymentDetailDispatcher = store => ({
  ...
  openModal: ({ type, url }) => store.dispatch({
    intent: OPEN_MODAL,
    modal: {
      type,
      url,
  ...
  loadInvoicePayment: response => store.dispatch({
    intent: LOAD_INVOICE_PAYMENT_DETAIL,
    ...response,
  }),
  updateInvoicePaymentDetails: ({ key, value }) => store.dispatch({
    intent: UPDATE_INVOICE_PAYMENT_DETAILS,
    key,
    value,
  }),
  updateShowPaidInvoices: value => store.dispatch({
  ...
  updateInvoicePaymentEntries: ({ name, value, index }) => store.dispatch({
    intent: UPDATE_INVOICE_PAYMENT_ENTRIES,
    name,
    value,
    index,
  }),
  loadInvoiceList: entries => store.dispatch({
