...
const getPayrollFavourites = createSelector(
  ...
  (all, favourites) => {
    const favouritePayrollReports = favourites.reduce((acc, name) => {
...
export const getViewAllUrl = createSelector(
  ...
  getRegion,
  getMyReportsUrl,
  (businessId, region, myReportsUrl) =>
    `${myReportsUrl}/#/${region}/${businessId}/reports/standardReports`
);
...
export const getFavouriteReports = createSelector(
  ...
  (businessId, region, myReportsUrl, favourites) =>
    favourites.map(({ displayName, path }) => ({
      displayName,
...
export const getPopularReports = createSelector(
  ...
  (businessId, region, myReportsUrl, favourites, all) => {
    const remainingReports = all.reduce((acc, allItem) => {
      const alreadyFavourite = favourites.find(
        (favouriteItem) => favouriteItem.name === allItem.name
      );
