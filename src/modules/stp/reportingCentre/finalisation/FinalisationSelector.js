...
export const getReportedSection57aRfba = (state) =>
  state.reportedSection57aRfba;
...
export const getIsRFBALocked = (state) =>
  Boolean(state.employees.find((e) => e.rfbAmount || e.s57aRfbAmount));
export const getIsRFBAEnabled = (state) =>
  state.isRFBAEnabled || getIsRFBALocked(state);
export const getShouldShowFinaliseButton = (state) =>
  Boolean(
    state.employees.find(
      (employee) => employee.isSelected && !employee.isFinalised
    )
  );
export const getShouldShowRemoveFinalisationButton = (state) =>
  Boolean(
    state.employees.find(
      (employee) => employee.isSelected && employee.isFinalised
    )
...
const getSelectedEmployees = (state) =>
  state.employees.filter((employee) => employee.isSelected);
export const getSubmitEmployeesFinalisationContent = (state) => ({
  eventId: getEventId(state),
  employees: getSelectedEmployees(state).filter(
    (employee) => !employee.isFinalised
  ),
});
