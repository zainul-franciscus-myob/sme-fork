...
export const getDebitAmount = (state) =>
  state.supplierReturnPurchase.debitAmount;
export const getDescription = (state) =>
  state.supplierReturnPurchase.description;
export const getReferenceId = (state) =>
  state.supplierReturnPurchase.referenceId;
export const getSupplierName = (state) =>
  state.supplierReturnPurchase.supplierName;
...
export const getIsCreating = (state) =>
  getSupplierReturnId(state) !== undefined;
...
export const getPageTitle = createSelector(
  ...
  (isCreating, referenceId) =>
    isCreating
      ? 'Apply supplier debit to purchase'
      : `Supplier debit applied to purchase ${referenceId}`
);

export const getIsTableEmpty = (state) =>
  state.supplierReturnPurchase.purchases.length === 0;
  ...
  (currentPurchases, region, businessId, isCreating) =>
    currentPurchases.map((purchase) => {
      ...
      return {
        ...
        amount: formatAmount(Number(purchase.amount) || 0),
        link,
      };
    })
);

const hasAmountPaidApplied = (amountApplied) =>
  amountApplied && amountApplied.length > 0 && amountApplied !== '0.00';
...
export const getSupplierReturnPurchasePayload = (state) => ({
  ...
  purchases: state.supplierReturnPurchase.purchases
    .filter((purchase) => hasAmountPaidApplied(purchase.amountApplied))
    .map((purchase) => ({
      id: purchase.id,
