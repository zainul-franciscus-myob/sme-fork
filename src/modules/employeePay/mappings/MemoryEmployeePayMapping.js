...
const MemoryEmployeePayMapping = {
  ...
  [SEND_EMPLOYEE_PAY_REVERSAL_MODAL]: ({ onSuccess }) => onSuccess({ message: 'Successfully send the reversal of the employee pay transaction' }),
  [SEND_EMPLOYEE_PAY_REVERSAL_DETAIL]: ({ onSuccess }) => onSuccess({ message: 'Successfully send the reversal of the employee pay transaction' }),
};
