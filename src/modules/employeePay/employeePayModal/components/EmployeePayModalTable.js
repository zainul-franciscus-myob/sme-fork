    ...
    .map((payItemGroup) => {
      ...
      const sectionBody = payItemGroup.items.map((item) => (
        <Table.Row key={item.payItemId}>
          ...
          <Table.RowItem {...tableConfig.hours}>{item.hours}</Table.RowItem>
          <Table.RowItem {...tableConfig.amount}>{item.amount}</Table.RowItem>
        </Table.Row>
      ));
    ...
    <Table>
      <Table.Header>
        ...
        <Table.HeaderItem {...tableConfig.hours}>
          {tableConfig.hours.columnName}
        </Table.HeaderItem>
        <Table.HeaderItem {...tableConfig.amount}>
          {tableConfig.amount.columnName}
        </Table.HeaderItem>
      </Table.Header>
      <Table.Body>{rowSections}</Table.Body>
    </Table>
  );
};

export default EmployeePayModalTable;
