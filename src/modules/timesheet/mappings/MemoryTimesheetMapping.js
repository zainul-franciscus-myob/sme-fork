...
const MemoryTimesheetMapping = {
  ...
  [LOAD_EMPLOYEE_TIMESHEET]: ({ onSuccess }) =>
    onSuccess(loadEmployeeTimesheet),
  [SAVE_TIMESHEET]: ({ onSuccess }) =>
    onSuccess({ message: 'Timesheet saved successfully!' }),
  [DELETE_TIMESHEET]: ({ onSuccess }) =>
    onSuccess({ message: 'Timesheet deleted successfully!' }),
};
