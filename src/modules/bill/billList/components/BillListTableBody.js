...
const BillListTableBody = (props) => {
  ...
  const rows = entries.map((entry) => (
    <Table.Row key={entry.id}>
      ...
      <Table.RowItem {...tableConfig.balanceDue}>
        {entry.balanceDueDisplayValue}
      </Table.RowItem>
      <Table.RowItem
      ...
      <Table.RowItem {...tableConfig.attachment}>
        {entry.hasAttachment ? <Icons.GenericDocument /> : ''}
      </Table.RowItem>
      <Table.RowItem {...tableConfig.status}>
        <Label type="boxed" color={entry.badgeColor}>
          {entry.status}
        </Label>
      </Table.RowItem>
    </Table.Row>
