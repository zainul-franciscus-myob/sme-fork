...
export default class EmployeePayListModule {
  ...
  changeEtpCodeCategory = ({ value }) =>
    this.dispatcher.changeEtpCodeCategory({
      etpCodeCategory: value,
    });

  changeEtpCode = ({ value }) =>
    this.dispatcher.changeEtpCode({
      etpCode: value,
    });
  ...
  loadDetailJobList = () => {
    ...
    if (getJobListModalLoadingState(state) === LoadingState.LOADING) {
      const onSuccess = (payload) => {
        this.dispatcher.setJobListModalLoadingState(
  ...
  validateEtp = (next) => {
    ...
    const onFailure = () => {
      ...
      this.dispatcher.setAlert({
        type: AlertType.ERROR,
        message: 'Failed to load validate ETP',
      });
    };
  ...
  updateJobs = ({ id, amount, isSelected }, jobs) => {
    ...
    return [...jobs.filter((q) => q.jobId !== id)];
  };
