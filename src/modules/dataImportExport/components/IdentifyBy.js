...
import { getContactIdentifyBy } from '../selectors/DataImportExportSelectors';
...
const ImportContactsDetail = ({
  ...
  >
    <Select.Option
      value={ContactIdentifyBy.NAME}
      label="Company name or last name"
    />
    <Select.Option value={ContactIdentifyBy.DISPLAY_ID} label="Contact ID" />
    <Select.Option
      value={ContactIdentifyBy.ID}
      label="Record ID - only for same company file"
    />
  </Select>
);
