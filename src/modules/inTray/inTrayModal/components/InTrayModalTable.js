...
const tableConfig = {
  ...
  uploadedDate: { columnName: 'Date uploaded', width: '14rem', valign: 'middle' },
  invoiceNumber: { columnName: 'Supplier invoice no.', width: 'flex-1', valign: 'middle' },
  issuedDate: { columnName: 'Issue date', width: '11rem', valign: 'middle' },
  amount: {
    columnName: 'Amount ($)', width: '13.5rem', valign: 'middle', align: 'right',
  },
...
const InTrayModalTable = ({
  ...
  const header = (
    <Table.Header>
      ...
      <Table.HeaderItem {...tableConfig.amount}>
        <HeaderSort title="Amount ($)" sortName="InvoiceAmount" activeSort={order} onSort={onSort} />
      </Table.HeaderItem>
  ...
  const dropZone = (
    <InTrayDropZone onUpload={onUpload} />
  );
