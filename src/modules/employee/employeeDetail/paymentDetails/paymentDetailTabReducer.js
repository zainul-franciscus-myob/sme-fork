...
const getUpdatedBankStatementText = (currentText, text) =>
  getAppliedFormatRestrictions(currentText, text, 18);

const getUpdatedAccountName = (currentText, text) =>
...
const updatePaymentDetails = (state, action) => ({
  ...
  paymentDetails: {
    ...
    [action.key]:
      action.key === 'bankStatementText'
        ? getUpdatedBankStatementText(
            state.paymentDetails.bankStatementText,
            action.value
          )
        : action.value,
  },
