...
export const getTaxPayItems = (state) => state.payrollDetails.tax.taxPayItems;

export const getFilteredTaxPayItemOptions = createSelector(
  getTaxPayItemOptions,
  getTaxPayItems,
  (taxPayItemOptions, taxPayItems) =>
    taxPayItemOptions.filter(
      (taxPayItemOption) =>
...
export const getSelectedTaxPayItems = createSelector(
  ...
  getTaxPayItems,
  (taxPayItemOptions, taxPayItems) =>
    taxPayItemOptions.filter((taxPayItemOption) =>
      taxPayItems.some((taxPayItem) => taxPayItem.id === taxPayItemOption.id)
    )
);
...
export const getSelectedTaxTable = (state) =>
  state.taxTableOptions.find(
    (taxTable) => taxTable.id === state.payrollDetails.tax.taxTableId
  ) || {};
...
export const getTaxFileNumberStatus = (state) =>
  state.payrollDetails.tax.taxFileNumberStatus;
export const getTaxPayItemModal = (state) => state.taxPayItemModal;
export const getTaxPayItemModalLoading = (state) =>
  state.taxPayItemModal.isLoading;
export const getTaxPayItemModalSubmitting = (state) =>
  state.taxPayItemModal.isSubmitting;
export const getTaxPayItemModalAlertMessage = (state) =>
  state.taxPayItemModal.alertMessage;

const getCurrentTfnState = createSelector(
  getPayrollDetailsTax,
  (payrollDetailTax) =>
    payrollDetailTax.taxFileNumberStates &&
...
export const getIsTfnEditable = (state) => {
  ...
  return tfnState ? tfnState.tfnEditable || false : false;
};

export const getHasTfn = (state) => {
  const tfnState = getCurrentTfnState(state);
  return tfnState ? tfnState.hasTfn || false : false;
};

export const getTfn = (state, tfnStatus) => {
  const tfnState = getTfnStateForStatus(state, tfnStatus);
  return tfnState ? tfnState.tfn || '' : '';
};
...
export const getTaxTableCalculations = (state) =>
  state.payrollDetails.tax.taxTableCalculation;
