...
export const getIsSuperPayItemModalCreating = state => state.superPayItemModal.id === 'new';

export const getTitle = state => state.superPayItemModal.title;

export const getSuperPayItemModalSuperPayItem = state => state.superPayItemModal.superPayItem;

const getContributionType = state => state.superPayItemModal.superPayItem.contributionType;

const getAtoReportingCategory = state => state.superPayItemModal.superPayItem.atoReportingCategory;

const getCalculationBasisType = state => state.superPayItemModal.superPayItem.calculationBasisType;

const getCalculationBasisPayItemId = state => state.superPayItemModal
  .superPayItem.calculationBasisPayItemId;

const getLimitPayItemId = state => state.superPayItemModal.superPayItem.limitPayItemId;

export const getEmployees = state => state.superPayItemModal.superPayItem.employees;
...
const getExemptionPayItemOptions = state => state.superPayItemModal.exemptionPayItemOptions;
...
export const getModalTitle = createSelector(
  ...
  getTitle,
  (isCreating, originalName) => (isCreating ? 'Create super pay item' : (originalName || 'Super pay item')),
);
...
export const getSuperPayItemDetail = createStructuredSelector({
  ...
  payableAccountId: state => state.superPayItemModal.superPayItem.payableAccountId,
  expenseAccountId: state => state.superPayItemModal.superPayItem.expenseAccountId,
  atoReportingCategory: state => state.superPayItemModal.superPayItem.atoReportingCategory,
  contributionType: state => state.superPayItemModal.superPayItem.contributionType,
  contributionTypeOptions: state => state.superPayItemModal.contributionTypeOptions,
  expenseAccountOptions: state => state.superPayItemModal.expenseAccountOptions,
  payableAccountOptions: state => state.superPayItemModal.payableAccountOptions,
...
export const getSuperPayItemInfo = createStructuredSelector({
  printOnPayAdvice: state => state.superPayItemModal.superPayItem.printOnPayAdvice,
...
export const getCalculationBasis = createStructuredSelector({
  ...
  calculationBasisPercentage: state => state.superPayItemModal
    .superPayItem.calculationBasisPercentage,
  calculationBasisPayItemId: getCalculationBasisPayItemId,
  calculationBasisAmount: state => state.superPayItemModal.superPayItem.calculationBasisAmount,
  calculationBasisPeriod: state => state.superPayItemModal.superPayItem.calculationBasisPeriod,
  exclusion: state => state.superPayItemModal.superPayItem.exclusion,
  calculationBasisTypeOptions: state => state.superPayItemModal.calculationBasisTypeOptions,
  calculationBasisPayItemOptions: getCalculationBasisPayItemOptions,
  periodOptions: getPeriodOptions,
  showPercent: state => state.superPayItemModal
    .superPayItem.calculationBasisType === calculationBasisTypes.percent,
  showAmount: state => state.superPayItemModal
    .superPayItem.calculationBasisType === calculationBasisTypes.amount,
});
...
export const getLimit = createStructuredSelector({
  ...
  limitPercentage: state => state.superPayItemModal.superPayItem.limitPercentage,
  limitPayItemId: state => state.superPayItemModal.superPayItem.limitPayItemId,
  ...
  periodOptions: getPeriodOptions,
  showPercent: state => state.superPayItemModal.superPayItem.limitType === limitTypes.percent,
  showAmount: state => state.superPayItemModal.superPayItem.limitType === limitTypes.amount,
});
...
export const getFilteredEmployeeOptions = createSelector(
  ...
  (employees, superPayItemEmployees) => employees.filter((employee) => {
    const listedItem = superPayItemEmployees
      .find(superPayItemEmployee => superPayItemEmployee.id === employee.id);
    return !listedItem;
  }),
);
...
export const getFilteredExemptionOptions = createSelector(
  ...
  getExemptions,
  (exemptionOptions, exemptions) => exemptionOptions.filter((option) => {
    const listedItem = exemptions
      .find(exemption => exemption.id === option.id);
    return !listedItem;
  }),
);
...
const getUpdatedAtoReportingCategory = (state) => {
  ...
  const filteredAtoReportingCategories = getFilteredAtoReportingCategoryOptions(state);
