...
export const getIsActionDisabled = (state) =>
  state.deductionPayItemModal.isSubmitting;
...
const getCalculationPercentOfId = (state) =>
  state.deductionPayItemModal.deductionPayItem.calculationPercentOfId;

const getEmployees = (state) =>
  state.deductionPayItemModal.deductionPayItem.employees;

const getExemptions = (state) =>
  state.deductionPayItemModal.deductionPayItem.exemptions;

const getLimitPercentOfId = (state) =>
  state.deductionPayItemModal.deductionPayItem.limitPercentOfId;

const getCalculationPercentOfOptions = (state) =>
  state.deductionPayItemModal.calculationPercentOfOptions;

const getCalculationDollarPerOptions = (state) =>
  state.deductionPayItemModal.calculationDollarPerOptions;

const getLimitPercentOfOptions = (state) =>
  state.deductionPayItemModal.limitPercentOfOptions;

export const getModalTitle = createSelector(
  getIsDeductionPayItemModalCreating,
  getTitle,
  (isCreating, title) =>
    isCreating ? 'Create deduction pay item' : title || 'Deduction pay item'
);

export const getDetails = createStructuredSelector({
  name: (state) => state.deductionPayItemModal.deductionPayItem.name,
  linkedPayableAccountId: (state) =>
    state.deductionPayItemModal.deductionPayItem.linkedPayableAccountId,
  atoReportingCategory: (state) =>
    state.deductionPayItemModal.deductionPayItem.atoReportingCategory,
  accountOptions: (state) => state.deductionPayItemModal.accountOptions,
  atoReportCategoryOptions: (state) =>
    state.deductionPayItemModal.atoReportCategoryOptions,
});

const getSelectedLimitPercentOf = createSelector(
  getLimitPercentOfId,
  getLimitPercentOfOptions,
  (limitPercentOfId, limitPercentOfOptions) =>
...
const getSelectedCalcPercentOf = createSelector(
  ...
  (calculationPercentOfId, calculationPercentOfOptions) =>
...
export const getInformation = createStructuredSelector({
  calculationBasis: (state) =>
    state.deductionPayItemModal.deductionPayItem.calculationBasis,
  calculationPercentage: (state) =>
    state.deductionPayItemModal.deductionPayItem.calculationPercentage,
  selectedCalculationPercentOf: getSelectedCalcPercentOf,
  calculationDollars: (state) =>
    state.deductionPayItemModal.deductionPayItem.calculationDollars,
  calculationPer: (state) =>
    state.deductionPayItemModal.deductionPayItem.calculationPer,
  calculationBasisOptions: (state) =>
    state.deductionPayItemModal.calculationBasisOptions,
  ...
  limitPercentage: (state) =>
    state.deductionPayItemModal.deductionPayItem.limitPercentage,
  selectedLimitPercentOf: getSelectedLimitPercentOf,
  limitDollars: (state) =>
    state.deductionPayItemModal.deductionPayItem.limitDollars,
  ...
  limitDollarPerOptions: (state) =>
    state.deductionPayItemModal.limitDollarPerOptions,
  isCalculationPercentage: (state) =>
    state.deductionPayItemModal.deductionPayItem.calculationBasis === 'Percent',
  isCalculationDollar: (state) =>
    state.deductionPayItemModal.deductionPayItem.calculationBasis ===
    'FixedDollar',
  isLimitDollar: (state) =>
    state.deductionPayItemModal.deductionPayItem.limit === 'FixedDollar',
  isLimitPercentage: (state) =>
    state.deductionPayItemModal.deductionPayItem.limit === 'Percent',
});
...
const isSelectedExemptionPayGWithholding = (state) =>
  state.deductionPayItemModal.deductionPayItem.exemptions.filter(
...
export const getDeductionPayItemModalPayload = (state) => {
  ...
  const { calculationPercentOfId, limitPercentOfId } = deductionPayItem;

  const calculationPercentOfType = (
    calculationPercentOfOptions.find(
      (option) => option.id === calculationPercentOfId
    ) || {}
  ).itemType;

  const limitPercentOfType = (
    limitPercentOfOptions.find((option) => option.id === limitPercentOfId) || {}
  ).itemType;
