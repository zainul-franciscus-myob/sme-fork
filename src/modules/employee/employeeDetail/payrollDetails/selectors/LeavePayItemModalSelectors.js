...
export const getLeavePayItemId = (state) =>
  state.leavePayItemModal.leavePayItemId;
...
export const getPrintOnPaySlip = (state) =>
  state.leavePayItemModal.leavePayItem.printOnPaySlip;
...
const getCalculationBasisType = (state) =>
  state.leavePayItemModal.leavePayItem.calculationBasisType;
...
export const getEnableExemptions = createSelector(
  ...
  (type, payItemId, configuration) =>
    isExemptionEnabled(type, payItemId, configuration)
);
...
export const getCalculationBasis = createStructuredSelector({
  ...
  calculationBasisAmount: (state) =>
    state.leavePayItemModal.leavePayItem.calculationBasisAmount,
  calculationBasisPeriod: (state) =>
    state.leavePayItemModal.leavePayItem.calculationBasisPeriod,
  calculationBasisTypes: (state) =>
    state.leavePayItemModal.calculationBasisTypes,
  ...
  showAmount: (state) =>
    state.leavePayItemModal.leavePayItem.calculationBasisType === 'FixedHours',
});

export const getLeavePayItemPayload = (state) => {
  const calculationBasisPayItem =
    state.leavePayItemModal.calculationBasisPercentOfOptions.find(
      ({ id }) =>
        id === state.leavePayItemModal.leavePayItem.calculationBasisPayItemId
    ) || {};
  ...
  return {
    ...
    selectedEmployees: state.leavePayItemModal.leavePayItem.selectedEmployees.map(
      ({ id }) => id
    ),
    selectedLinkedWages: state.leavePayItemModal.leavePayItem.selectedLinkedWages.map(
      ({ id }) => id
    ),
  };
};
