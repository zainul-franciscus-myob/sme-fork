...
const PayrollDetailSuperannuationTable = ({
  ...
  const tableBodyView = allocatedPayItems.map(({ id, name, displayType }) => (
    <Table.Row key={id}>
      <Table.RowItem {...tableConfig.name}>
        <Button
          type="link"
          onClick={onPayItemSelect(onOpenSuperPayItemModal, id)}
        >
          {name}
        </Button>
      </Table.RowItem>
        ...
        <Tooltip
          ...
          triggerContent={
            <Button
              type="secondary"
              size="xs"
              onClick={onRemoveButtonClick(onRemovePayrollSuperPayItem, id)}
            >
              <Icons.Remove />
            </Button>
          }
  ...
  const payItemCombobox = showAddPayItemButton ? (
