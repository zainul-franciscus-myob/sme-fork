...
describe('PayrollWageReducer', () => {
  ...
  describe('updatePayrollWageHourlyRate', () => {
    it.each([
      ...
      'should calculate annual salary for a %s pay cycle',
      (payCycle, expectedAnnualSalary) => {
        const state = {
          payrollDetails: {
            wage: {
              payPeriodHours: '10',
  ...
  describe('updatePayrollWageAnnualSalary', () => {
    it.each([
      ...
      'should calculate hourly rate for a %s pay cycle',
      (payCycle, payPeriodHours, expectedHourlyRate) => {
        const state = {
          payrollDetails: {
            wage: {
              payPeriodHours,
  ...
  describe('updatePayrollWageHoursInPayCycle', () => {
    it.each([
      ...
      'should calculate %s for %s hours on %s basis for %s pay cycle',
      (
        _,
        hours,
        payBasis,
  ...
  describe('updatePayrollWagePayCycle', () => {
    it.each([
      ...
      'should update hours in pay cycle when changed from %s to %s',
      (origPayCycle, newPayCycle, hoursPerPeriod, expectedHoursPerPeriod) => {
        const state = {
          payrollDetails: {
            wage: {
    ...
    it.each([
      ...
      'should update hourly rate when pay basis is %s',
      (selectedPayBasis, expectedAnnualSalary, expectedHourlyRate) => {
        const state = {
          payrollDetails: {
            wage: {
              selectedPayBasis,
        ...
        const { annualSalary, hourlyRate } = employeeDetailReducer(
          state,
          action
        ).payrollDetails.wage;
