import { Badge, Button, Icons, Table } from '@myob/myob-widgets';
...
const ItemListTableBody = ({
  ...
  const emptyView = (
    <NoResultPageState
      ...
      description={
        isFilteredList
          ? 'Perhaps check spelling or remove filters and try again'
    ...
    >
      {entries.map((entry) => (
        <Table.Row key={entry.id}>
          ...
          {getStatusRow(entry)}
          <Table.RowItem {...tableConfig.sellingPrice}>
            {entry.sellingPrice}
          </Table.RowItem>
