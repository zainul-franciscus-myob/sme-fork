...
export const getCurrentEmployeeName = state => (state.currentEmployee ? state.currentEmployee.name : '');
export const getCurrentEmployeeEmail = state => (state.currentEmployee ? state.currentEmployee.email : '');
...
export const getEmailContentForCurrentEmployee = (state) => {
  const {
    fromEmail, fromName, paySlipBody, paySlipSubject,
  } = state.emailSettings;
...
export const getIsFinished = state => getEmployeeCount(state) === getTotalEmployees(state);
