import {
  Alert, Button, Icons, Input, Modal, Spinner, Tooltip,
...
const UploadOptionsModal = ({
  ...
  const confirmEmailGenerationButtonIcon = (
    <Tooltip placement="right" triggerContent={<Icons.Info />}>
      Use if you&apos;re having problems, such as irrelevant documents being added
    </Tooltip>
  );
  ...
  const emailGenerationInfoAlert = (
    <Alert type="info" onDismiss={onDismissConfirmEmailGeneration}>
      Are you sure you want to generate a new email address? New documents sent to the old
      address will no longer appear here.&nbsp;
  ...
  const modalBody = (
    <Modal.Body>
      ...
      <h3>Email</h3>
      <p>
        Send documents to the email address below and they&apos;ll automatically appear in the
        In Tray. You can share the email address with your suppliers too.
      </p>

      <h3>Your In Tray email address:</h3>
      <div className={style.inTrayEmailAddress}>
        <Input disabled hideLabel label="Email" name="email" value={email} data-copy />
        <Button type="secondary" onClick={onCopyEmailButtonClicked}>Copy</Button>
      </div>
      { isConfirmingEmailGeneration ? emailGenerationInfoAlert : confirmEmailGenerationButton }
      ...
      <p>
        Use the MYOB Capture app to scan and send documents to In Tray.
      </p>
    </Modal.Body>
  );
  ...
  return (
    <Modal title="More ways to upload documents" onCancel={onCancel}>
      ...
      <Modal.Footer>
        <Button disabled={isUploadOptionsLoading} type="secondary" onClick={onCancel}>Close</Button>
      </Modal.Footer>
    </Modal>
  );
};
