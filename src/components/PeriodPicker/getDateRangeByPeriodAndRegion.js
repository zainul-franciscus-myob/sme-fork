...
const isBetweenJanAndMar = (date) =>
  isBefore(date, startOfDay(new Date(getYear(date), 3, 1)));

const isBeforeJuly = (date) =>
  isBefore(date, startOfDay(new Date(getYear(date), 6, 1)));

const isBeforeOct = (date) =>
  isBefore(date, startOfDay(new Date(getYear(date), 9, 1)));
...
const getLastFinancialYear = (date, region) =>
  region === 'nz'
    ? getFinancialYearNZ(subYears(date, 1))
    : getFinancialYearAU(subYears(date, 1));
